 ...
 * notes:
   * 仿函数不是函数，它是一个类
   *仿函数重载了（）运算符，使它的对你可以像函数那样子调用
 *  作用：就是将函数对象当作算法参数进行调用
 *  分类：
   * 算术类仿函数
      * plus<T>:加法 
      * minus<T>：减法。。。
   ...
   * 证同、选择、投射
      * 证同函数（identity<T>）
         * 任何数值通过此函数后，不会有任何的变化
      * 选择函数（select1st<pair>，select2nd<pair>）
         * 接受一个pair，传回其第一个元素
         * 接受一个pair，传回其第二个元素
      * 投射函数（project1st<Arg1,Arg2>,project2nd<Arg1,Arg2>）
         * 传回第一参数，忽略第二参数
         * 传回第二个参数，忽略第一参数
 ...
 ## 配接器
  ...
  简介：配接器在STL组件的灵活组合运用功能上，扮演着轴承，转换器的角色，即将一个class的借口转换为另一个class的接口，使原本因接口不兼容而不能合作的clsses，可以一起运行，事实上是一种设计模式
 ...
 * 容器配接器
   * STL提供的容器queue和stack，其实都只不过是一种配接器，它们修饰deuqe的接口而成就出另一种容器风貌
 * 迭代器配接器
   * inset_iterators
      * 可以将一般迭代器的赋值操作转变为插入操作
   * reverse_iterators 
      * 可以将一般迭代器的行进方向逆转，使原本应该前进的operatir++变成了
   * iostream_iterators
      * 可以将迭代器绑定到某个iostream对象上
 * 仿函数配接器
   * 配接操作包括系结（bind）,否定(negate),组合(compose)，以及对一般函数或者成员函数的修饰（使其成为一个仿函数)
   * 通过组合仿函数，形成另一个仿函数
   * 所有期望获得配接能力的组件，本身都必须是可配接的。即，一元仿函数都必须继承自unary_function，二元仿函数必须继承自binary_function
